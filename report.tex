...
\begin{figure}[h]
  ...
  \caption{MCMC trace plots after step-size tuning. The chain should mix
           well with acceptance rate near 23\%.}
  \label{fig:sl_trace}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Synthetic likelihood MCMC posterior (800 post-burn-in samples
         after optimal step-size tuning).}
\label{tab:sl}
\begin{tabular}{lrrrrrr}
\toprule
...
The SL chain uses step sizes derived from the optimal scaling rule
\citep{roberts1997}; results should be re-evaluated after re-running
the notebook with the updated configuration.

\begin{table}[h]
\centering
\caption{Posterior standard deviations across all methods.
         $^\dagger$Results pending re-run with tuned step sizes.}
\label{tab:compare}
\begin{tabular}{lrrrr}
\toprule
...
Synth.\ lik.$^\dagger$ & 0.003 & 0.002 & 0.007 & Step sizes tuned \\
\bottomrule
\end{tabular}
\end{table}
